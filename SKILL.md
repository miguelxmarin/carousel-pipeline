...
| Google Drive PDF upload | YES | `upload_to_drive.py` uses Google Drive API v3 with OAuth2. One-time 3-min setup (see below), then fully automated forever. Sets "Anyone with the link -- Viewer" sharing automatically. |
...
python scripts/upload_to_drive.py --slot-dir posts/YYYY-MM-DD/HHMM    # upload PDF to Drive, set sharing, save link to carousel.json
...
This ensures it can be sent to anyone (DM, comment reply, email) and they can
open and download the PDF without being logged into Google.
