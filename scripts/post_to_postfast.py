...
def select_x_slides(keys: list[str]) -> list[str]:
    """
    X only supports 4 images. Pick: hook (1), problem (3), solution (5), CTA (9).
    Falls back gracefully if the carousel has fewer than 9 slides.
    """
    total = len(keys)
    indices = [0, 2, 4, total - 1]          # slides 1, 3, 5, last
    seen, result = set(), []
    for i in indices:
...
def build_payload(
    ...
    keys_x: list[str],          # 4 slides → X only
    caption: str,
    scheduled_at: str,
    post_now: bool,
...
def post_slot(slot_dir: Path, target_date: date, dry_run=False, post_now=False, lang="en"):
    ...
    if x_id:         print(f"  X:         {x_id}  (4-slide: hook/problem/solution/CTA)")
    ...
    # X gets a 4-slide synthesis: hook (1), problem (3), solution (5), CTA (9)
    keys_x = select_x_slides(keys)
    if x_id:
        print(f"  X slides selected: {len(keys_x)}/4 (indices 1,3,5,last)")
